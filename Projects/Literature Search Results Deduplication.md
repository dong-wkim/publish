Python script to deduplicate search results based on column(s) of interest and generate flowchart diagrams with numbers required to be reported by PRISMA.
...
df = pd.read_csv('search_export.csv')
columns = df[['title', 'author']]
